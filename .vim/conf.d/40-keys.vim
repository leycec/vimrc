...
" * cmap() and cnoremap(), recursively and nonrecursively defining a mapping
...
"   for insert mode.
" * smap() and snoremap(), recursively and nonrecursively defining a mapping
"   for select mode.
" * vmap() and vnoremap(), recursively and nonrecursively defining a mapping
"   for visual *AND* select modes.
" * xmap() and xnoremap(), recursively and nonrecursively defining a mapping
"   for visual mode.
"
...
"
" In general, prefer nonrecursive to recursive mappings. While the former
" behave exactly as specified, the behavior of the latter dynamically change
...
" * Custom key bindings, pass that binding to the ":map" Ex command (e.g.,
"   ":map ,dd"). Whereas printable characters (e.g., "j") are expressed as
"   undelimited case-insensitive arguments, non-printable characters (e.g.,
"   "Ctrl-space") *MUST* be expressed as "<"- and ">"-delimited
...
" Bind <,nh> to either:
"
" * If a window exists to the left of the current window, switch to that
"   window.
