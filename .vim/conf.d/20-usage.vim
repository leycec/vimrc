...
" --------------------( WASTELANDS                         )--------------------
"FUXME: Unfortunately, this *STILL* appears to induce syntax highlighting
"failures on long docstrings -- and we have no idea why. We've tried literally
"*EVERYTHING* to correct this, with no success. Since this appears to be
"unfixable at the moment, we sadly accept such unsightliness and move on.
...
" let g:vimen_cache_dir = expand('~/.vim/cache')
" Prefix the passed pathname by the absolute path of the directory to cache
" files to.
" function vimen#get_cache_path(pathname)
"     return g:vimen_cache_dir . a:pathname
" endfunction
...
" endif
    " return resolve(expand(s:cache_dir . '/' . a:pathname))

    "FUXME: Slightly inefficient. autocommands targetting single filetypes (as
    "most of the following do) should be shifted to "ftplugin/"-specific files.
    "Why? Because this lazily prevents such autocommands from even being
    "evaluated until a buffer of such filetype is opened, speeding up startup.

    " For HTML files, reduce such defaults for improved readability (e.g., when
    " editing deeply nested tag structures).

    " configuring such filetypes already do so. Since such logic depends upon
    " filetypes established above, perform such autocommands *AFTER* prior
    " autocommands. The following code-specific filetypes are omitted:

    " Avoid autowrapping code by disabling option "t". This includes:
    "
    " * Markdown. While conventional Markdown ignores newlines and hence permits
    "   contiguous text in the same paragraph to be delimited by ignorable
    "   newlines, GitHub's increasingly popular variant of Markdown does *NOT*
    "   ignore newlines. For simplicity, assume all Markdown to adhere to the
    "   latter standard.

    "FUXME: We've temporarily removed our enabling of formatoptions "a". Why?
    "Because we currently 
    " * "a", autoformatting all paragraphs of contiguous text.
    " Disable comment autoformatting for Markdown files. While conventional
    " Markdown ignores newlines and hence permits contiguous text in the same
    " paragraph to be delimited by ignorable newlines, GitHub's increasingly
    " popular variant of Markdown does *NOT* ignore newlines. For simplicity,
    " assume all Markdown to adhere to the latter standard.
    " autocmd FileType markdown setlocal formatoptions-=t

"FUXME: Dangerous when set globally. Set on a filetype basis.
" set formatoptions+=jn
" Disable text autoformatting.
" set formatoptions-=t

"  Such filetypes
" are identifiable by running the Ex command
" ":set fo"; if the output resembles "formatoptions=tcq", such filetype
" does *NOT* autoformat text.
" Bind <Ctrl-h>, <Ctrl-j>, <Ctrl-k>, and <Ctrl-l> to navigate to the
" corresponding windows (i.e., without requiring the vexing intermediary "w").
" noremap <C-h> <C-w>h
" noremap <C-j> <C-w>j
" noremap <C-k> <C-w>k
" noremap <C-l> <C-w>l

      " \ autocmd BufEnter,InsertLeave,TextChanged * WatchdogsRun
" Note the duplication of the prior list of filetypes, albeit in comma-delimited
" form. The VimL-jutsu is weak with this one. Perhaps some braggadocio Redditor –
" *maybe you, yes* – will supply a fix. (DRY principle, yuh8me?)
"FUXME: The hightlight color is, frankly, terrible. Please correct.
" if !exists('g:quickrun_config')
"     let g:quickrun_config = {}
" endif
"FUXME: We *REALLY* want to enable the following statement. To do so, however,
"we'll need to uncover how to integrate watchdogs with airline:

" * On buffer writes (e.g., ":w").
"       \ autocmd BufNewFile,BufRead,BufWritePost,InsertLeave,TextChanged *
"FUXME: Document. Find similar settings at the online documentation:
"    https://github.com/osyo-manga/vim-watchdogs/blob/master/doc/watchdogs.jax

" ....................{ SYNTAX CHECK ~ syntastic           }....................
" Syntastic provides blacklist-style synchronous syntax checking. Note that, as
" this is the inverse of watchdogs, the two play nicely together.
"
" To list both all Syntastic-specific syntax checkers applicable to the current
" buffer *AND* the subset of checkers currently applied to such buffer, run:
"     :SyntasticInfo

" Check syntax on opening buffers with filetypes registered with Syntastic.
" let g:syntastic_check_on_open = 1
"let g:syntastic_quiet_warnings = 1  " Only errors, not warnings please

" Enable active rather than passive syntax checking (i.e., check syntax on
" every buffer open and save), excluding buffers whose filetype is in the set
" of "passive_filetypes" defined below. Such buffers default to passive syntax
" checking, in which case the user must explicitly call :SyntasticCheck to
" enable syntax checking. This includes the following filetypes:
"
" * "python" and "vim", for which "vim-watchdogs" provides asynchronous and
"   hence fundamentally superior syntax checkers.
" * "zeshy", for which no syntax checker and hence Syntastic plugin exists.
" let g:syntastic_mode_map = {
"   \ 'mode': 'active',
"   \ 'passive_filetypes': ['python', 'vim', 'zeshy'] }

" ....................{ FILETYPE ~ code                    }....................
"FUXME: Ugh. Some piece of intelligent (!) code somewhere is coercing
"formatoptions" to "tcq" for *ALL* filetypes, which is terrible. Investigate.

" augroup filetype_code
"     autocmd!
"     autocmd FileType zeshy,vim setlocal formatoptions+=lor formatoptions-=t
" augroup END

" Common Syntastic commands include:
"let g:syntastic_enable_signs = 1    " Put errors on left side
" Denote errors and warnings with Unicode symbols.
