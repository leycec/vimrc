...
" ....................{ PROMPT                            }....................
" Sanitize prompt settings (e.g., Vim's audible and visual response to events).
" (Default prompt settings are sufficiently insane to warrant sanitization
" *BEFORE* performing any subsequent logic.)
...
" ....................{ GLOBALS                           }....................
" 1 if Vim is currently starting up or 0 otherwise (i.e., Vim has already
" started up).
let g:our_is_startup = 1
...
"   startup is typically a lost cause. Subsequently printed strings (e.g., by
"   third-party plugins of lower priority) take precedence over previously
"   printed strings, obscuring or even outright squelching strings printed
"   during early startup. Likewise, as highlight groups are mostly unavailable 
"   during early startup, such strings would mostly be unformatted, further
"   contributing to their obscuration by subsequent output.
" * Else, this string is printed immediately.
function PrintError(message) abort
    " If Vim is currently starting up...
    if g:our_is_startup
        " Escape all single quotes in the passed string, permitting this string
        " to be safely embedded in single quotes below.
        let l:message_escaped = substitute(a:message, "'", "''", 'g')

        " Defer printing this string until after Vim has started up. For
        " unknown and presumably nonsensical reasons, attempting to call the
        " echoerr() builtin here fails; ergo, we instead:
        "
        " * Temporarily enable the error-specific highlight group.
        " * Print the passed message as a simple message.
        " * Revert back to a non-highlighted group.
        execute
          \ 'autocmd VimEnter * '
          \ 'echohl ErrorMsg | '
          \ 'echomsg ''' . l:message_escaped . ''' | '
          \ 'echohl None'
    " Else, Vim has already started up. In this case, print this message as is.
    else
        echoerr a:message
    endif
endfunction
...
" str GetPathablePath(str pathable)
"
...
" ....................{ CHECKS                            }....................
" If this is NeoVim, then this Vim environment already supports all requisite
" features (e.g., asynchronous job support); else, this is stock Vim, in which
" case these features are only available under sufficiently new versions of
" Vim. In the latter case, if the current version of Vim is insufficient, print
" a non-fatal warning.
"
...
" * "dein", requiring either NeoVim or Vim >= 7.4.2071 as a hard prerequisite.
" * "vim-gitgutter":
"   * Recommending Vim >= 7.4.427 to avoid random highlighting glitches.
"   * Requiring Vim >= 7.3.105 for realtime highlighting.
if ! g:our_is_nvim && v:version < 742
    if v:version < 800
        call PrintError(
          \ 'Vim version older than 8.0 detected. Expect relative horror.')
    else
        call PrintError(
          \ 'Vim version older than 7.4 detected. Expect absolute horror.')
    endif
    ...
    "
    " For other platforms, upgrading Vim is usually trivial and hence omitted.
    if g:our_is_platform_macos
        " If Homebrew is unavailable, recommend its installation.
        if !executable('brew')
            call PrintError('Consider installing Homebrew to correct this.')
        endif
...
" is 1 if Vim was compiled with job control *AND* is either NeoVim (all
" versions of which provide asynchronous job control out-of-the-box) or Vim 8
" or newer.
...
if !has('autocmd')
    call PrintError('Vim feature "autocmd" unavailable. Expect terror.')
endif
if !has('signs')
    call PrintError('Vim feature "signs" unavailable. Expect ugliness.')
endif
...
" * Under Ubuntu, uninstall the "vim" package and install the "vim-gtk3"
"   package.
if g:our_is_display_server_x11 && !has('clipboard')
    " If under macOS, only the "+clipboard" feature is typically enabled. The
...
if g:our_is_platform_windows_vanilla && !executable('mingw32-make')
    call PrintError(
      \ 'Command "mingw32-make" not found. Expect dein installation to fail.')
" Else if "make" is not in the current ${PATH}, print a non-fatal warning.
" While generally unlikely, "make" is *NOT* installed under non-vanilla
" Cygwin-enabled Microsoft Windows by default. The "vimproc" plugin runs this
" command to compile itself under all platforms that are *NOT* vanilla
" Microsoft Windows.
elseif !executable('make')
    call PrintError(
      \ 'Command "make" not found. Expect dein installation to fail.')
endif
...
augroup our_filetype_detect
    ...
    " These are vcsh-specific files in canonical ".gitignore" format, which Vim
